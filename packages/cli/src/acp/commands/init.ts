...
export class InitCommand implements Command {
  name = 'init';
  description = 'Analyzes the project and creates a tailored SEGA.md file';
  ...
  async execute(
    ...
    switch (result.type) {
      ...
      case 'submit_prompt':
        ...
        // or just seed the SEGA.md file as we've done above.
        return {
          name: this.name,
          data: {
            type: 'message',
            messageType: 'info',
            content: `A template SEGA.md has been created at ${geminiMdPath}.\n\nTo populate it with project context, you can run the following prompt in a new chat:\n\n${result.content}`,
          },
        };
