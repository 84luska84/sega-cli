  ...
  describe('/memory list', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        ...
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} SEGA.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No SEGA.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
    ...
    it('should display a message if no SEGA.md files are found', async () => {
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/SEGA.md', '/path/two/SEGA.md'];
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 SEGA.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
